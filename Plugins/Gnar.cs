...
namespace AIM.Plugins
{
    public class Gnar : PluginBase
    {
        ...
        public void OnCombo(Obj_AI_Hero target)
        {
            ...
            else
            {
                ...
                if (RMega.IsReady())
                {
                    ...
                    if (prediction.Hitchance >= HitChance.High && RMega.IsInRange(prediction.UnitPosition))
                    {
                        ...
                        while (true)
                        {
                            ...
                            // Rotated check
                                ...
                                checkPoint = prediction.UnitPosition + 500 * Rotated(direction, currentAngle);

                            // Check for a wall between the checkPoint and the target position
                            if (GetFirstWallPoint(prediction.UnitPosition,checkPoint).HasValue)
                            {
        ...
        public Vector3? GetFirstWallPoint(Vector3 from, Vector3 to, float step = 25)
        {
        ...
        public Vector2? GetFirstWallPoint(Vector2 from, Vector2 to, float step = 25)
        {
            var direction = (to - from).Normalized();
        ...
        public Vector3 Rotated(Vector3 v, float angle)
        {
