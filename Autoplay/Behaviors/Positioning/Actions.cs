    ...
    internal class Actions
    {
        ...
        internal BehaviorAction ProtectFarthestTurret = new BehaviorAction(
            () =>
            {
                ...
                var allyNexus = ObjectManager.Get<Obj_HQ>().FirstOrDefault(hq => hq.IsAlly);
                var objConstants = new Constants();
                var orbwalkingPos = new Vector2();
                ...
                {
                    orbwalkingPos.X = allyNexus.Position.X + (objConstants.DefensiveAdditioner / 8f) +
                                      Randoms.Rand.Next(-100, 100);
                    orbwalkingPos.Y = allyNexus.Position.Y + (objConstants.DefensiveAdditioner / 8f) +
                                      Randoms.Rand.Next(-100, 100);
                }
